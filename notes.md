...
login W&B with API
4.
python main.py --data data  --model gin --emlps --reverse_mp --ports --save_model --unique_name "edge"
...
12. train test split is based on time. we want to map it back to oringal ids, not time id.



#### EXPORTING THE MODEL TO ONNX fORMAT (FOR WATSONX)

Refer to the article here 
https://dataplatform.cloud.ibm.com/docs/content/wsj/analyze-data/deploy-batch-input-pytorch.html?context=wx&audience=wdp&locale=en


```python format_model.py --inference --data data --model gin --emlps --reverse_mp --ports  --unique_name "edge"```

Notes
- Wrapper needed as ONNX requires tensor/list input, while hetero GNN model (from torch geometric) requires dict input.